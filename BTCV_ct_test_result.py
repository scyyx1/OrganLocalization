...
ground_truth_path = ""
result_path = ""
model_num = 2 # 0 for raw model, 1 for gradient model, 2 for mulGM model
if model_num == 0:
    # BTCV raw model
    file_output_directory = "Dataset/BTCV_data/Testing/"
    ground_truth_path = "Dataset/BTCV_data/Testing/img/ground_truth/test-segmentation-"
    result_path = "Dataset/BTCV_data/Testing/img/results/test-volume-"
elif model_num == 1:
    # BTCV gradient model
    file_output_directory = "Dataset/BTCV_data/GradientTesting/"
    ground_truth_path = "Dataset/BTCV_data/GradientTesting/img/ground_truth/test-segmentation-"
    result_path = "Dataset/BTCV_data/GradientTesting/img/results/test-volume-"
elif model_num == 2:
    # BTCV mulGM model
    file_output_directory = "Dataset/BTCV_data/MulGMTesting/"
    ground_truth_path = "Dataset/BTCV_data/MulGMTesting/img/ground_truth/test-segmentation-"
    result_path = "Dataset/BTCV_data/MulGMTesting/img/results/test-volume-"
...
organs_dict = {1: liver, 4: kidney_r, 5: kidney_l, 8: bladder, 10: spleen, 11: pancreas}

# Record Each Organ's Number in Ground Truth
ground_truth_organ_num = {"liver": 0, "kidney-r": 0, "kidney-l": 0, "bladder": 0, "spleen": 0, "pancreas": 0}
...
ite = 30 # Number of Testing Images
for i in range(ite):
...
detected_organs = 5 # Number of Detected Organs (Here we exclude bladder)
# Print Accuracy with Detected Organs
print("IoU with Total Detected Organs in Results")
global_accuracy = (liver_accuracy+kidney_r_accuracy+kidney_l_accuracy+
                   bladder_accuracy+spleen_accuracy+pancreas_accuracy)/detected_organs
...
global_accuracy_gt = (liver_accuracy_gt+kidney_r_accuracy_gt +kidney_l_accuracy_gt
                      + bladder_accuracy_gt+spleen_accuracy_gt+
                      pancreas_accuracy_gt) / detected_organs
...
global_detection_rate = (liver_detection_rate+kidney_r_detection_rate +kidney_l_detection_rate
                          +spleen_detection_rate+pancreas_detection_rate) / detected_organs
